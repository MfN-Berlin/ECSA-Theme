...
foreach ( $newItems as $k => $item ) {
  $newItems[$k]['slide'] = '<div class="ui grid"><div class="doubling two column row">' . $item['slide'] . '</div></div>' ;
}

?>
<div class="events mobile-einser">
	<a href="<?php //echo $_SERVER['REQUEST_URI']; ?><?php global $base_url; print $base_url;?>/<?php global $language_url; print $language_url->language;?>/taxonomy/term/128"><h2 class="pane-title"><?php print t('Upcoming events');?> <i class="calendar outline icon"><span class="calendar-date"><?php print date('j');?></span></i></h2></a>
